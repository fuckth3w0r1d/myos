...
{
    _intr_status old_status = _disable_intr();     //关中断避免产生冲突，因为我们需要实现多线程
    ...
    elem->next = before;
    before->prev = elem;
    _set_intr_status(old_status);      //恢复以前的中断状态
}
...
struct list_elem* list_traversal(struct list* plist, function func, int arg)
{
    ...
    while(elem != &plist->tail)
    {
        if(func(elem, arg)) return elem; //返回true，则认为该元素在回调函数中符合条件，命中，故停止继续遍历
        elem = elem->next;
    }
    return NULL;
}
