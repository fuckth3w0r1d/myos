...
struct _task_struct* idle_thread;    //idle线程
...
struct _task_struct* thread_create(char* name, uint8_t prio, thread_func function, void* func_arg)
{
    ...
    thread_init_info(thread, name, prio);
    thread_init_stack(thread, function, func_arg);
    // 确保之前不在队列中
...
{
   //在loader.S中进入内核时的mov esp,0xc009f000,就是为其预留PCB，所以PCB的地址就是0xc009e000 不需要另分配一页
    main_thread = running_thread();
    thread_init_info(main_thread, "main", 31);
