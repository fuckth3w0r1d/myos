...
static bool mount_partition(struct list_elem* pelem, int arg)
{
    ...
    if(!strcmp(part_name, part->name))
    {
        ...
        cur_part->block_bitmap.bytes_len = sb_buf->block_bitmap_sects * SECTOR_SIZE;
        ...
        cur_part->inode_bitmap.bytes_len = sb_buf->inode_bitmap_sects * SECTOR_SIZE;
            ...
            {   //共有4个主分区和8个逻辑分区
                ...
                if(part->sec_cnt != 0){   //如果分区存在
                    ...
                    if(sb_buf->magic == 0x20001109)
                    {
                        ...
                        kprintf("formatting %s's partition %s.......\n", hd->name, part->name);
...
static char* path_parse(char* pathname, char* name_store)
{
    ...
    *name_store = '\x00';
    if(pathname[0] == 0) return NULL; //如果路径字符串为空，则返回NULL
    return pathname;
}
...
int32_t sys_open(const char* pathname, uint8_t flags)
{
    ...
    {
        kprintf("can not open a dirctory %s\n", pathname);
        return -1;
    }
    ASSERT(flags <= 7);
    int32_t fd = -1;  //默认为找不到
    ...
    {
        kprintf("can't open a directory with open(),use opendir to instead\n");
        dir_close(searched_record.parent_dir);
        return -1;
    }
    ...
    {
        kprintf("cannot access %s: Not a directory, subpath %s is not exist\n", pathname, searched_record.searched_path);
        dir_close(searched_record.parent_dir);
        return -1;
    }
    ...
    {
        kprintf("in path %s, file %s is't exist\n", searched_record.searched_path, (strrchr(searched_record.searched_path, '/') + 1));
        dir_close(searched_record.parent_dir);
        return -1;
    }else if(found && flags & O_CREAT){
        kprintf("%s has already exist\n", pathname);
        dir_close(searched_record.parent_dir);
        return -1;
    }
...
int32_t sys_close(int32_t fd)
{
    ...
    if(fd > 2)
    {
        ...
        running_thread()->fd_table[fd] = -1;    //置空被占用的描述符
    }
    return ret;
}
