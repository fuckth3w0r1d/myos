...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos)
{
    ...
    uint32_t off_size = inode_no * (uint32_t)sizeof(struct inode);    //inode_no号I结点相对于inode_table_lba的字节偏移量
    ...
    if(left_in_sec < (uint32_t)sizeof(struct inode))
...
void inode_sync(struct partition* part, struct inode* inode, void* io_buf)
{
    ...
    {   //若是横跨两个扇区，则需要读两个扇区写两个扇区, 读写硬盘是以扇区为单位
...
struct inode* inode_open(struct partition* part, uint32_t inode_no)
{
    ...
    uint32_t* cur_thread_pgdir = cur->pgdir;   //临时保存用户页目录
    cur->pgdir = NULL;
    // 现在我们分配内存就会从内核空间分配了
    inode_found = (struct inode*)sys_malloc(sizeof(struct inode));
    // 恢复pgdir
    cur->pgdir = cur_thread_pgdir;
    ...
    if(inode_pos.two_sec)
    {
        ...
        inode_buf = (char*)sys_malloc(512);     //一个扇区
...
void inode_close(struct inode* inode)
{
    ...
    if(--inode->i_open_cnts == 0)
    {
        ...
        struct _task_struct* cur = running_thread();
        uint32_t* cur_thread_pgdir = cur->pgdir;
        cur->pgdir = NULL;
        sys_free(inode);
        cur->pgdir = cur_thread_pgdir;
    }
