...
bool search_dir_entry(struct partition* part, struct dir* pdir, const char* name, struct dir_entry* dir_e)
{
    ...
    if(all_blocks == NULL)
    {
        kprintf("search_dir_entry: sys_malloc for all_blocks failed");
        return false;
    }
...
void dir_close(struct dir* dir)
{
    ...
    //root_dir所在的内存是低端1MB之内，并非在堆中，free会出问题
        ...
        if(all_blocks[block_idx] == 0)
        {
            ...
            {     //1.如果是直接块
                ...
                {   //分配失败，回滚
                    ...
                    _set_bitmap(&cur_part->block_bitmap, block_bitmap_idx, 0);
