...
// 定义可读写的最大扇区数
#define max_lba ((128*1024*1024/512) - 1)    //支持128MB

uint8_t channel_cnt;        //按硬盘数计算的通道数
struct ide_channel channels[2];     //有两个ide通道

// 用于记录总扩展分区的起始lba，初始为0,scan_partition时以此为标记
...
// 向硬盘控制器写入起始扇区地址及要读写的扇区数 (lba地址就可以理解为扇区索引)
static void set_sector_args(struct disk* hd, uint32_t lba, uint8_t sec_cnt)
{
    ASSERT(lba <= max_lba);
...
// 向通道发出命令cmd
static void send_cmd_to_channel(struct ide_channel* channel, uint8_t cmd)
{
...
static void identify_disk(struct disk* hd)
{
    char id_info[512];
    select_disk(hd);
    send_cmd_to_channel(hd->my_channel, CMD_IDENTIFY);
    // 阻塞自己，等待硬盘准备好数据再唤醒
    sema_down(&hd->my_channel->disk_done);
    // 醒来后执行以下代码
    if(!busy_wait(hd))
...
static void scan_partition(struct disk* hd, uint32_t ext_lba)
{
...
void _init_ide()
{
    ...
    uint8_t hd_cnt = *((uint8_t*)0x475);    //获取硬盘数量，这个地址是固定的
    ASSERT(hd_cnt > 0);
    ...
    while(channel_no < channel_cnt)
    {
        ...
        // 此信号量初始化为0,目的是向硬盘控制器请求数据后，因盘驱动sema_down此信号会阻塞线程，
        //直到硬盘完成后通过发中断，由中断处理程序将此信号sema_up，唤醒线程
        sema_init(&channel->disk_done, 0);
        _register_intr_handler(channel->irq_no, _hd_intr_handler);
        // 分别获取两个个硬盘的参数及分区信息
        while(dev_no < 2)
        {
